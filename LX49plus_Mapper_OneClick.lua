...
-- Mapper simplifié pour REAPER + Nektar Impact LX49+
-- Un seul ReaScript à charger/lancer : il installe le bridge JSFX, crée la piste de contrôle,
-- ajoute le FX, puis ouvre l'interface de mapping.

local function __lx49_path_join(a, b)
  local sep = package.config:sub(1, 1)
  if a:sub(-1) == sep then return a .. b end
  return a .. sep .. b
end

local __LX49_BRIDGE_JSFX = [=[desc:LX49+ CC Bridge to ReaScript
// Captures incoming MIDI CC messages and publishes them to a shared gmem ring buffer.
// This file is written automatically by LX49plus_Mapper_OneClick.lua.
...
local function __lx49_write_text_file(path, content)
...
local function __lx49_find_or_create_control_track()
  local wanted = "LX49+ CONTROL"
  for i = 0, reaper.CountTracks(0) - 1 do
    local tr = reaper.GetTrack(0, i)
    local _, name = reaper.GetSetMediaTrackInfo_String(tr, "P_NAME", "", false)
    local low = (name or ""):lower()
    if low == wanted:lower() or (low:find("lx49", 1, true) and low:find("control", 1, true)) then
      return tr, false
    end
  end
  ...
  reaper.GetSetMediaTrackInfo_String(tr, "P_NAME", wanted, true)
  return tr, true
end

local function __lx49_find_midi_input()
  local n = reaper.GetNumMIDIInputs and reaper.GetNumMIDIInputs() or 0
  local best_idx, best_name = nil, nil
  for i = 0, n - 1 do
    local ok, name = reaper.GetMIDIInputName(i, "")
    if ok and name and name ~= "" then
      local low = name:lower()
      if low:find("lx49", 1, true) or (low:find("impact", 1, true) and low:find("lx", 1, true)) or low:find("nektar", 1, true) then
...
local function __lx49_track_has_bridge(tr)
  for i = 0, reaper.TrackFX_GetCount(tr) - 1 do
    local _, fxname = reaper.TrackFX_GetFXName(tr, i, "")
    local low = (fxname or ""):lower()
    if low:find("lx49", 1, true) and low:find("bridge", 1, true) then
      return i
    end
  end
  return -1
...
local function __lx49_add_bridge_fx(tr)
  local existing = __lx49_track_has_bridge(tr)
  if existing >= 0 then return existing end

  local candidates = {
    "JS: LX49+ CC Bridge to ReaScript",
    "LX49+ CC Bridge to ReaScript",
    "LX49plus_CC_Bridge",
    "JS: LX49plus_CC_Bridge"
  }
...
local function __lx49_setup()
  local messages = {}
  local resource = reaper.GetResourcePath()
  local effects_dir = __lx49_path_join(resource, "Effects")
  reaper.RecursiveCreateDirectory(effects_dir, 0)

  local bridge_path = __lx49_path_join(effects_dir, "LX49plus_CC_Bridge.jsfx")
  local ok, err = __lx49_write_text_file(bridge_path, __LX49_BRIDGE_JSFX)
  if ok then
    messages[#messages + 1] = "Bridge JSFX installé automatiquement."
  else
    messages[#messages + 1] = "Erreur écriture JSFX : " .. tostring(err)
  end

  local tr, created = __lx49_find_or_create_control_track()
  if tr then
    ...
    local input_idx, input_name = __lx49_find_midi_input()
    if input_idx then
      -- REAPER MIDI record input encoding: 4096 + device_index * 32 + channel. Channel 0 = all channels.
      reaper.SetMediaTrackInfo_Value(tr, "I_RECINPUT", 4096 + (input_idx * 32))
      messages[#messages + 1] = "Piste LX49+ CONTROL prête, entrée MIDI : " .. input_name
    else
      messages[#messages + 1] = "Piste LX49+ CONTROL prête. Sélectionne l'entrée MIDI LX49+ manuellement si aucun CC n'arrive."
    end

    local fx = __lx49_add_bridge_fx(tr)
    if fx >= 0 then
      messages[#messages + 1] = "FX bridge ajouté sur la piste de contrôle."
    else
      messages[#messages + 1] = "JSFX copié, mais pas ajouté. Redémarre REAPER puis relance ce script."
    end

    reaper.TrackList_AdjustWindows(false)
    reaper.UpdateArrange()
  else
    messages[#messages + 1] = "Impossible de créer la piste de contrôle."
  end

  return table.concat(messages, "  ")
end

_G.__LX49_SETUP_MESSAGE = __lx49_setup()
...
local message = _G.__LX49_SETUP_MESSAGE or "Bouge un potard/fader du LX49+ pour vérifier la réception MIDI."
